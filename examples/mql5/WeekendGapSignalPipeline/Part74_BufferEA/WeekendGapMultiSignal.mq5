...
bool      g_firstRun    = true;                       // First-run flag
...
//--- Place arrows 2 pips away from the candle edge to avoid overlap
...
//--- Iterate through bars to find weekend gaps
...
//--- Set series orientation for easy indexing
...
//--- Detect a new bar (closed bar)
   bool newBar = (time[0] != g_lastBarTime);
   if(newBar)
      g_lastBarTime = time[0];

//--- First run or full recalculation: build the gap database and historical signals
...
//--- On a new bar, check for new gaps and live signals
   if(newBar)
     {
      //--- Detect a potential new weekend gap using closed bars only
      ...
      //--- Update the visual appearance of all gaps
      if(ShowHistoricalGaps)
        {
         for(int g = 0; g < ArraySize(g_gaps); g++)
